...
function isKnownTopLevelCategory(value: string) {
  return new Set(["KIDS", "LEARNING", "GAME", "BUSINESS", "PORTFOLIO", "ECOMMERCE", "BLOG", "LANDING", "OTHER"]).has(normalizeKey(value));
}

export default async function GameFilters({ selectedAge, selectedType, selectedCategory = "ALL", dynamicAgeOptions = [], dynamicCategoryOptions = [] }: Props) {
  let dbAgeOptions = dynamicAgeOptions;
  let dbCategoryOptions = dynamicCategoryOptions;

  try {
    const gameSites = await prisma.website.findMany({
      where: { isPublished: true, OR: [{ category: "GAME" }, { age: { not: null } }, { gameType: { not: null } }] },
      select: { age: true, category: true, gameType: true },
    });
    ...
    const categoryValues = [...new Set(
      gameSites
        ...
        .filter((value): value is string => Boolean(value) && normalizeKey(value) !== "GAME" && !isKnownTopLevelCategory(value))
    )];

    dbAgeOptions = [...dbAgeOptions, ...ageValues.map((value) => ({ key: value, label: labelForAge(value) }))];
    dbCategoryOptions = [...dbCategoryOptions, ...categoryValues.map((value) => ({ key: value, label: value }))];
  } catch (error) {
  ...
  const mergedCategories = [
    ...
    ...dbCategoryOptions.filter((option, index, all) => all.findIndex((item) => normalizeKey(item.key) === normalizeKey(option.key)) === index),
  ];
    ...
    <section className="mt-5 rounded-[28px] border border-sky-200/80 bg-gradient-to-br from-sky-50/90 via-white/80 to-indigo-50/80 p-5 shadow-[0_20px_55px_-30px_rgba(14,116,144,0.42)] backdrop-blur-xl dark:border-sky-500/20 dark:from-sky-950/25 dark:via-slate-950/40 dark:to-indigo-950/25 sm:p-6">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">Choose an age group, category, and game style. Custom ages and game categories added by the admin appear here automatically.</p>
        </div>
        ...
        <div className="rounded-2xl border border-white/80 bg-white/65 p-4 dark:border-white/10 dark:bg-slate-950/30">
          ...
          <div className="flex flex-wrap gap-2">
            {mergedCategories.length > 1 ? mergedCategories.map((option) => {
              const active = selectedCategory === normalizeKey(option.key);
