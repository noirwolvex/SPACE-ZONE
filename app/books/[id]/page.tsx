...
/**
 * `path` is deliberately absent from every selection on this page: the storage
 * reference for the PDF must never be serialized into the HTML. Reads happen
 * through /api/books/[id]/access, which authorizes each request.
 */
...
export async function generateMetadata({
  ...
  const book = await prisma.book.findUnique({
    where: { id },
    select: { title: true, filename: true, summary: true },
  });

  if (!book) return { title: "Book not found — Space Zone" };

  return {
    title: `${book.title ?? book.filename} — Space Zone`,
...
export default async function BookDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Presentation only — /api/books/[id]/access re-checks this on every read.
  ...
  // Only signed URLs cross into the client; storage references stay here.
    ...
    <main className="relative flex-1 overflow-hidden bg-linear-to-b from-slate-50 via-indigo-50/40 to-white px-4 py-8 text-slate-900 transition-colors sm:px-6 lg:px-8 dark:from-[#050505] dark:via-[#070b18] dark:to-[#050505] dark:text-white">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-8">
        ...
        <nav aria-label="Breadcrumb" className="animate-fade-in flex items-center gap-1.5 text-sm">
          <Link
            href="/books"
            className="group inline-flex items-center gap-1.5 font-semibold text-slate-500 transition-colors hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-300"
          >
        ...
        <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:gap-12 lg:items-start">
          ...
          {/* Sticky so the purchase action stays reachable while reading. */}
          <div className="flex flex-col gap-6 lg:sticky lg:top-6">
            <header className="animate-fade-up">
              <div className="flex flex-wrap items-center gap-2">
                {rest.category ? (
                  <Link
                    href={`/books${buildBooksQueryString({ categories: [rest.category] })}`}
                    className="rounded-full border border-indigo-200 bg-indigo-50 px-3.5 py-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-indigo-700 transition-all duration-300 hover:-translate-y-0.5 hover:border-indigo-400 hover:shadow-sm dark:border-indigo-500/30 dark:bg-indigo-950/60 dark:text-indigo-200"
                  >
                    {rest.category}
                  </Link>
                ) : null}
              ...
              <h1 className="mt-4 text-4xl font-black leading-[1.1] tracking-tight text-slate-950 sm:text-5xl dark:text-white">
                {displayTitle}
              </h1>

              {rest.author ? (
                <p className="mt-3 text-lg text-slate-600 dark:text-slate-300">
                  by <span className="font-bold text-slate-900 dark:text-white">{rest.author}</span>
                </p>
              ) : null}
            </header>
            ...
            <section className="animate-fade-up" style={{ animationDelay: "0.14s" }}>
              <h2 className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.25em] text-slate-500 dark:text-slate-400">
                <span className="h-px w-6 bg-indigo-400" />
                About this book
              </h2>
              <p className="mt-3.5 whitespace-pre-line text-base leading-8 text-slate-700 dark:text-slate-300">
                {rest.summary ?? "A delightful book from the Space Zone library."}
              </p>
            </section>
            ...
            {facts.length > 0 ? (
              ...
              >
                {facts.map((fact) => (
                  <div
                    key={fact.label}
                    className="rounded-2xl border border-slate-200 bg-white/70 p-4 transition-all duration-300 hover:-translate-y-1 hover:border-indigo-300 hover:shadow-md dark:border-slate-800 dark:bg-slate-900/50 dark:hover:border-indigo-500/40"
                  >
                    <dt className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                      {fact.label}
                    </dt>
                    <dd className="mt-1.5 truncate text-sm font-bold text-slate-900 dark:text-white">
                      {fact.value}
                    </dd>
                  </div>
                ))}
