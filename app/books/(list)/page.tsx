...
export default async function BooksPage({
  ...
  const [books, categoryGroups, totalBooks, freeBooks] = await Promise.all([
    ...
    // Chips are derived from categories actually present in the catalogue, so a
    // category filter can never come up empty by construction. Counts are
    // unfiltered on purpose: they describe the library, not the current view.
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-8">
          ...
          <div className="grid gap-0 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="p-8 sm:p-10 lg:p-12">
              ...
              <p className="mt-5 max-w-2xl text-base leading-8 text-slate-600 sm:text-lg dark:text-slate-300">
                Discover a carefully selected collection of books with elegant presentation and a smooth
                reading experience.
              </p>
              ...
              <dl className="mt-8 grid grid-cols-3 gap-3 sm:max-w-lg">
                {stats.map((stat) => (
                  <div
                    key={stat.label}
                    className="group rounded-2xl border border-slate-200 bg-white/70 p-4 transition-all duration-300 hover:-translate-y-1 hover:border-indigo-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-950/50 dark:hover:border-indigo-500/40"
                  >
                    <stat.icon className="h-4 w-4 text-indigo-500 transition-transform duration-300 group-hover:scale-110 dark:text-indigo-400" />
                    <dd className="mt-2 text-2xl font-black tabular-nums text-slate-950 dark:text-white">
                      {stat.value}
                    </dd>
                    <dt className="mt-0.5 text-[11px] font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
                      {stat.label}
                    </dt>
                  </div>
                ))}
            ...
            <div className="relative hidden overflow-hidden bg-slate-950 p-8 sm:p-10 lg:block lg:p-12">
              ...
              <div className="relative rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-md">
                ...
                <div className="mt-5 space-y-3">
                  {[
                    "Beautiful cover presentation that gives each title presence",
                    "Clear details for age, category, and reading access",
                    "A calm, premium feel that makes browsing enjoyable",
                  ].map((item) => (
                    <div
                      key={item}
                      className="flex items-start gap-3 rounded-2xl border border-white/10 bg-slate-900/50 p-3.5 transition-all duration-300 hover:translate-x-1 hover:border-indigo-400/40 hover:bg-slate-900/80"
                    >
        ...
        {booksWithAccess.length === 0 ? (
          <div className="animate-fade-up flex flex-col items-center rounded-[28px] border border-dashed border-slate-300 bg-white/70 p-14 text-center shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/50">
            ...
            <p className="mt-5 text-lg font-bold text-slate-800 dark:text-slate-100">
              {hasActiveFilters(filters) ? "No books match these filters" : "No books available yet"}
            </p>
            <p className="mt-2 max-w-sm text-sm text-slate-500 dark:text-slate-400">
              {hasActiveFilters(filters)
                ? "Try removing one of the active filters above, or clear them all to see the full library."
                : "New titles are on their way — check back soon."}
            </p>
          </div>
        ) : (
          <section className="stagger-children grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
            {booksWithAccess.map((book) => (
