...
const initialForm: WebsiteFormValues = {
  title: "", summary: "", description: "", system: "", details: "", features: "", targetAudience: "", responsive: "Fully Responsive", age: "", gameType: "", category: "", price: "0", currency: "BHD", websiteUrl: "", isPublished: true
};
...
export default function AdminWebsitesPage() {
  ...
  function prepareEdit(w: WebsiteRecord) { setEditingId(w.id); setForm({ title: w.title || "", summary: w.summary || "", description: w.description || "", system: w.system || "", details: w.details || "", features: w.features || "", targetAudience: w.targetAudience || "", responsive: w.responsive || "Fully Responsive", age: w.age || "", gameType: w.gameType || "", category: w.category || "", price: String(w.price ?? 0), currency: w.currency || "BHD", websiteUrl: w.websiteUrl || "", isPublished: Boolean(w.isPublished) }); setSelectedImageFiles([]); setImagePreviews([]); setStatus("Editing website. Choose images only if you want to replace the gallery."); }
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/40 dark:shadow-none">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-medium"><span className="mb-2 block">Age</span><select name="age" value={form.age || ""} onChange={handleFieldChange} disabled={form.category.trim().toUpperCase() !== "GAME"} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 disabled:cursor-not-allowed disabled:opacity-50 dark:border-indigo-500/30 dark:bg-slate-950"><option value="">Select age</option>{WEBSITE_GAME_AGES.map((age) => <option key={age} value={age}>{age === "3-5" ? "Ages 3–5" : age === "6-8" ? "Ages 6–8" : age === "9-12" ? "Ages 9–12" : "Ages 13+"}</option>)}</select><span className="mt-1 block text-xs text-slate-500">Used by the Games age filter.</span></label>
            <label className="text-sm font-medium"><span className="mb-2 block">Game Type</span><select name="gameType" value={form.gameType || ""} onChange={handleFieldChange} disabled={form.category.trim().toUpperCase() !== "GAME"} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 disabled:cursor-not-allowed disabled:opacity-50 dark:border-indigo-500/30 dark:bg-slate-950"><option value="">Select game type</option>{WEBSITE_GAME_TYPES.map((type) => <option key={type} value={type}>{type.charAt(0) + type.slice(1).toLowerCase()}</option>)}</select><span className="mt-1 block text-xs text-slate-500">Used by the Games type filter.</span></label>
