...
const initialForm: WebsiteFormValues = { title: "", summary: "", description: "", system: "", details: "", category: "", price: "0", currency: "BHD", websiteUrl: "", isPublished: true };
...
export default function AdminWebsitesPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      data.append("title", form.title); data.append("summary", form.summary || ""); data.append("description", form.description || ""); data.append("system", form.system || ""); data.append("details", form.details || ""); data.append("category", form.category); data.append("price", String(form.price || "0")); data.append("currency", form.currency || "BHD"); data.append("websiteUrl", form.websiteUrl); data.append("isPublished", String(Boolean(form.isPublished)));
  ...
  function prepareEdit(w: WebsiteRecord) { setEditingId(w.id); setForm({ title: w.title || "", summary: w.summary || "", description: w.description || "", system: w.system || "", details: w.details || "", category: w.category || "", price: String(w.price ?? 0), currency: w.currency || "BHD", websiteUrl: w.websiteUrl || "", isPublished: Boolean(w.isPublished) }); setSelectedImageFiles([]); setImagePreviews([]); setStatus("Editing website. Choose images only if you want to replace the gallery."); }
  ...
  return (
    <main className="flex-1 bg-slate-50 px-4 py-10 text-slate-900 transition-colors dark:bg-[#050505] dark:text-white">
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-white px-3 py-1.5 text-sm font-semibold text-indigo-700 shadow-sm dark:border-indigo-500/30 dark:bg-slate-900/60 dark:text-indigo-300"><ShieldCheck className="h-4 w-4" />Space Zone Admin — Websites</div>
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/40 dark:shadow-none">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-medium"><span className="mb-2 block">Category</span><select name="category" value={form.category} onChange={handleFieldChange} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-indigo-500/30 dark:bg-slate-950"><option value="">Select a category</option>{WEBSITE_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select></label>
            ...
            <label className="md:col-span-2 text-sm font-medium"><span className="mb-2 block">Description</span><textarea name="description" value={form.description} onChange={handleFieldChange} rows={5} placeholder="Full project description" className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-indigo-500/30 dark:bg-slate-950" /></label>
            ...
            <label className="md:col-span-2 text-sm font-medium"><span className="mb-2 block">Other Details</span><textarea name="details" value={form.details} onChange={handleFieldChange} rows={5} placeholder="Features, pages, included services, notes..." className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-indigo-500/30 dark:bg-slate-950" /></label>
