...
const initialForm: WebsiteFormValues = {
  ...
  responsive: "Fully Responsive", age: "", customAge: "", gameType: "", category: "", customCategory: "",
...
export default function AdminWebsitesPage() {
  ...
  async function submit() {
    const category = effectiveCategory; const age = effectiveAge;
    ...
    if (category.toUpperCase() === "GAME" && !form.gameType) return setStatus("Game type is required for game websites.");
    setIsLoading(true); setStatus("");
    try {
      ...
      data.append("gameType", form.gameType || ""); data.append("category", category); data.append("price", String(form.price || "0"));
  ...
  function prepareEdit(w: WebsiteRecord) {
    ...
    setForm({
      ...
      age: ageParts.preset, customAge: ageParts.custom, gameType: w.gameType || "", category: categoryParts.preset, customCategory: categoryParts.custom,
      price: String(w.price ?? 0), currency: w.currency || "BHD", websiteUrl: w.websiteUrl || "", isPublished: Boolean(w.isPublished),
    });
    setSelectedImageFiles([]); setImagePreviews([]); setStatus("Editing website. Custom Age and Custom Category can be used for your own values.");
  }
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/40 dark:shadow-none">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div><h2 className="text-xl font-bold">Manage Websites</h2><p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Create, edit, publish, and manage website details shown on the public page.</p></div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <label className="md:col-span-2 text-sm font-medium"><span className="mb-2 block">Description</span><textarea name="description" value={form.description} onChange={handleFieldChange} rows={10} placeholder="Full project description" className={`${inputClass} min-h-64 resize-y`} /></label>
            <label className="md:col-span-2 text-sm font-medium"><span className="mb-2 block">Features</span><textarea name="features" value={form.features} onChange={handleFieldChange} rows={7} placeholder="List the main website features, one per line." className={`${inputClass} min-h-52 resize-y`} /></label>
            <label className="md:col-span-2 text-sm font-medium"><span className="mb-2 block">Other Details</span><textarea name="details" value={form.details} onChange={handleFieldChange} rows={7} placeholder="Additional pages, included services, notes..." className={`${inputClass} min-h-52 resize-y`} /></label>
