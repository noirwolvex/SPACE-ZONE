...
export default function AdminDashboard() {
  ...
  function editTool(tool: AdminTool) {
    ...
    setToolForm({
      ...
      bestFor: listToText(tool.bestFor),
    });
  }
  ...
  async function saveService(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(
      isEditing
        ? `/api/admin/services/${serviceForm.id}`
        : "/api/admin/services",
      {
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-8 flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/40 dark:shadow-none md:flex-row md:items-center">
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 transition hover:bg-slate-100 dark:border-indigo-500/30 dark:bg-[#0a0f1e] dark:text-slate-200 dark:hover:bg-indigo-950/40"
          >
        ...
        <section className="grid gap-4 md:grid-cols-2">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/40 dark:shadow-none"
            >
              ...
              <p className="mt-4 text-3xl font-extrabold text-slate-950 dark:text-white">
                {stat.value}
              </p>
            </div>
          ))}
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-bold transition ${
              activeTab === "services"
                ...
                : "border border-slate-300 bg-white text-slate-800 dark:border-indigo-500/30 dark:bg-slate-900 dark:text-slate-200"
            }`}
          >
            Services
          </button>
          ...
          <a
            href="/admin/books"
            className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-800 transition hover:bg-slate-50 dark:border-indigo-500/30 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
          >
            Books
          </a>
        ...
        {activeTab === "services" ? (
          <section className="mt-6 grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
            <form
              ...
              className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/40 dark:shadow-none"
            >
              ...
              <AdminInput
                ...
                placeholder="Designing Store Banners"
              />
              ...
              <button
                type="submit"
                className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-indigo-600 px-5 py-3 text-sm font-bold text-white transition hover:bg-indigo-500"
              >
                {isLoading ? (
            ...
            >
              <div className="mb-5 flex items-center justify-between gap-4">
                ...
                {toolForm.id ? (
                  <button
                    type="button"
                    onClick={() => setToolForm(emptyToolForm)}
                    className="inline-flex items-center gap-1 text-sm font-semibold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
                  >
                    <X className="h-4 w-4" />
                    Cancel
              ...
              <AdminTextarea
                label="Included files, one per line"
                optional
                value={toolForm.includedFiles}
                onChange={(value) =>
                  setToolForm({
                    ...toolForm,
                    includedFiles: value,
              ...
              <AdminTextarea
                label="Best for, one per line"
                optional
                value={toolForm.bestFor}
                onChange={(value) =>
                  setToolForm({
                    ...toolForm,
                    bestFor: value,
  ...
  return (
    <label className="mt-4 block text-sm font-semibold text-slate-700 dark:text-slate-200">
      ...
      <input
        ...
        className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-slate-950 outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 dark:border-indigo-500/30 dark:bg-slate-950 dark:text-white dark:focus:ring-indigo-950"
      />
    </label>
  );
}
      ...
      {items.length ? (
        <div className="space-y-4">
          {items.map((item) => (
            <article
              key={item.slug}
              className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-indigo-500/20 dark:bg-slate-950/40"
            >
              <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  <button
                    ...
                    disabled={!item.id}
                    className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-red-500/30 dark:bg-red-950/30 dark:text-red-200"
                  >
