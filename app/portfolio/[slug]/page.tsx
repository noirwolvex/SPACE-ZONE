...
function Block({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/50 md:p-8"><h2 className="text-2xl font-bold text-slate-950 dark:text-white">{title}</h2><div className="mt-4 leading-8 text-slate-600 dark:text-slate-300">{children}</div></section>;
}
  ...
  return <main className="flex-1 bg-slate-50 pt-24 pb-16 text-slate-900 dark:bg-[#050505] dark:text-white"><div className="container mx-auto max-w-6xl px-4">
    ...
    <section className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/50">
      ...
      <div className="p-6 sm:p-8 lg:p-12"><div className="max-w-4xl"><p className="text-xs font-bold uppercase tracking-[0.25em] text-indigo-600 dark:text-indigo-300">{project.industry||project.tags[0]||"Case Study"}</p><h1 className="mt-3 text-4xl font-extrabold tracking-tight text-slate-950 dark:text-white md:text-6xl">{project.title}</h1><p className="mt-6 text-xl leading-9 text-slate-600 dark:text-slate-300">{project.summary}</p></div>
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Industry",project.industry,Target],["Duration",project.duration,Clock3],["Services",services.slice(0,2).join(" · "),Layers3],["Tools",project.techStack.slice(0,2).join(" · "),Wrench]].map(([label,value,Icon])=>value?<div key={String(label)} className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-indigo-500/20 dark:bg-slate-950/40"><Icon className="h-5 w-5 text-indigo-600 dark:text-indigo-300"/><p className="mt-3 text-xs font-bold uppercase tracking-[0.18em] text-slate-500">{label}</p><p className="mt-1 font-semibold">{String(value)}</p></div>:null)}</div>
        {metrics.length?<div className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-4">{metrics.map(m=><div key={`${m.label}-${m.value}`} className="rounded-2xl border border-indigo-200 bg-indigo-50 p-5 dark:border-indigo-500/20 dark:bg-indigo-950/30"><p className="text-xs font-semibold text-indigo-700 dark:text-indigo-300">{m.label}</p><p className="mt-1 text-2xl font-extrabold text-slate-950 dark:text-white">{m.value}</p></div>)}</div>:null}
      </div>
    </section>
    ...
    {project.process.length?<section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/50 md:p-8"><h2 className="text-2xl font-bold">How we built it</h2><div className="mt-6 grid gap-4 md:grid-cols-2">{project.process.map((step,index)=><div key={`${step}-${index}`} className="flex gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-indigo-500/20 dark:bg-slate-950/40"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-sm font-black text-white">{index+1}</span><p className="leading-7 text-slate-700 dark:text-slate-300">{step}</p></div>)}</div></section>:null}
    ...
    {project.techStack.length?<section className="mt-8 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-indigo-500/20 dark:bg-slate-900/50 md:p-8"><h2 className="text-2xl font-bold">Technology & tools</h2><div className="mt-5 flex flex-wrap gap-2">{project.techStack.map(item=><span key={item} className="rounded-full border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700 dark:border-indigo-500/30 dark:bg-indigo-950/40 dark:text-indigo-300">{item}</span>)}</div></section>:null}
