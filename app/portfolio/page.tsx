...
async function getSelectedWork(): Promise<DisplayProject[]> {
  try {
    const records = await prisma.portfolioProject.findMany({ orderBy: { createdAt: "asc" } });
    if (!records.length) return portfolioProjects;
    ...
    <main className="flex-1 bg-slate-50 pt-24 pb-16 text-slate-900 transition-colors dark:bg-[#050505] dark:text-white">
      <div className="container mx-auto max-w-6xl px-4">
        <section className="grid gap-10 border-b border-slate-200 pb-14 dark:border-indigo-900/30 lg:grid-cols-[1fr_0.72fr] lg:items-end">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href="/contact" className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-[0_4px_15px_rgba(79,70,229,0.2)] transition hover:bg-indigo-500">Start a Project<ArrowRight className="h-4 w-4" /></Link>
              <a href="https://www.instagram.com/spacezonemedia/" target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-800 transition hover:bg-slate-100 dark:border-indigo-500/30 dark:bg-[#0a0f1e] dark:text-slate-200"><ExternalLink className="h-4 w-4" />View Instagram</a>
            </div>
          </div>
        ...
        <section className="py-14">
          <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><h2 className="text-3xl font-bold tracking-tight">Selected Work</h2><p className="mt-3 max-w-2xl text-slate-600 dark:text-slate-300">Projects managed from the admin portfolio workspace and shown here automatically.</p></div><div className="flex flex-wrap gap-2">{portfolioCapabilities.slice(0,3).map((capability)=><span key={capability} className="rounded-full border border-slate-200 bg-white px-3 py-1 text-sm font-medium dark:border-indigo-500/20 dark:bg-slate-900/50">{capability}</span>)}</div></div>
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {selectedWork.map((project) => (
              <article key={project.id ?? project.title} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md transition duration-300 hover:border-indigo-400/50 hover:shadow-[0_0_25px_rgba(79,70,229,0.15)] dark:border-indigo-500/20 dark:bg-slate-900/40">
                <div className="relative min-h-48 overflow-hidden bg-gradient-to-br p-0 ${project.gradient}">
                  {project.gallery?.[0] ? <img src={project.gallery[0]} alt={project.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : null}
                  <div className="relative m-6 rounded-2xl border border-white/70 bg-white/70 p-4 shadow-lg backdrop-blur dark:border-white/10 dark:bg-slate-950/55"><p className="text-xs font-bold uppercase tracking-wider text-indigo-700 dark:text-indigo-300">{project.category}</p><h3 className="mt-2 text-xl font-extrabold text-slate-950 dark:text-white">{project.title}</h3></div>
                </div>
                <div className="p-6"><p className="text-sm leading-6 text-slate-600 dark:text-slate-300">{project.summary}</p>{project.metrics?.length>0&&<div className="mt-5 grid grid-cols-2 gap-3">{project.metrics.map((metric)=><div key={`${metric.label}-${metric.value}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-indigo-500/20 dark:bg-slate-950/40"><p className="text-xs text-slate-500">{metric.label}</p><p className="mt-1 text-sm font-bold">{metric.value}</p></div>)}</div>}<div className="mt-5 flex flex-wrap gap-2">{project.services.slice(0,4).map((service)=><span key={service} className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300">{service}</span>)}</div></div>
              </article>
            ))}
          </div>
        </section>
