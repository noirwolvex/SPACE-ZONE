    ...
    <main className="flex-1 bg-slate-50 pt-24 pb-16 text-slate-900 transition-colors dark:bg-[#050505] dark:text-white">
      <div className="container mx-auto max-w-6xl px-4">
        ...
        <section className="py-14">
          ...
          {selectedWork.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white/70 p-12 text-center text-slate-500 dark:border-indigo-500/20 dark:bg-slate-900/50 dark:text-slate-400">No portfolio projects have been added yet.</div> : <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">{selectedWork.map((project)=><Link href={`/portfolio/${project.slug}`} key={project.id} className="group block min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-md transition hover:-translate-y-1 hover:border-indigo-400/60 hover:shadow-xl dark:border-indigo-500/20 dark:bg-slate-900/40"><div className={`relative min-h-48 min-w-0 overflow-hidden bg-gradient-to-br ${project.gradient} p-0`}>{project.gallery[0]?<Image src={project.gallery[0]} alt={project.title} fill sizes="(max-width: 1024px) 50vw, 33vw" className="object-cover transition duration-500 group-hover:scale-105"/>:null}<div className="relative m-6 min-w-0 rounded-2xl border border-white/70 bg-white/70 p-4 shadow-lg backdrop-blur dark:border-white/10 dark:bg-slate-950/55"><p className="break-words text-xs font-bold uppercase tracking-wider text-indigo-700 [overflow-wrap:anywhere] dark:text-indigo-300">{project.category}</p><h3 className="mt-2 break-words text-xl font-extrabold text-slate-950 [overflow-wrap:anywhere] dark:text-white">{project.title}</h3></div></div><div className="min-w-0 p-6"><p className="break-words text-sm leading-6 text-slate-600 [overflow-wrap:anywhere] dark:text-slate-300">{project.summary}</p>{project.outcome?<p className="mt-4 break-words text-sm leading-6 text-slate-600 [overflow-wrap:anywhere] dark:text-slate-300">{project.outcome}</p>:null}{project.metrics.length>0&&<div className="mt-5 grid grid-cols-2 gap-3">{project.metrics.map((metric)=><div key={`${metric.label}-${metric.value}`} className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-indigo-500/20 dark:bg-slate-950/40"><p className="break-words text-xs text-slate-500 [overflow-wrap:anywhere]">{metric.label}</p><p className="mt-1 break-words text-sm font-bold [overflow-wrap:anywhere]">{metric.value}</p></div>)}</div>}<div className="mt-5 flex flex-wrap gap-2">{project.services.slice(0,4).map((service)=><span key={service} className="max-w-full break-words rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700 [overflow-wrap:anywhere] dark:bg-indigo-950/40 dark:text-indigo-300">{service}</span>)}</div><p className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-indigo-700 dark:text-indigo-300">View project <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></p></div></Link>)}</div>}
        </section>
